...
function nama_user($id)
{
  ...
  $nama_user = $ci->db->get_where('users', ['id' => $id])->row();
  return $nama_user->first_name . " " . $nama_user->last_name;
}
function nama_pelanggan($id)
{
  $ci = &get_instance();
  $ci->load->model('Data_pelanggan_model');
  $nama_user = $ci->db->get_where('Data_pelanggan', ['id_pelanggan' => $id])->row();
  return $nama_user->nama_pelanggan;
}

function cek_keterangan_validasi($status)
{
  switch ($status) {
    case '0':
      return "<span class='badge' style='background-color:#dc3545'>Sudah divalidasi, Data tidak sesuai</span>";
      break;
    case '1':
      return "<span class='badge' style='background-color:#198754'>Sudah divalidasi, Data sesuai</span>";
      break;
  }
}

function cek_validasi_data($valid)
{
  switch ($valid) {
    case '0':
